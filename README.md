...
あなたの仕事に寄り添った仕組みづくり「たすかるワーク」のWebサイトです。

## ポジショニング・SEO方針（2026-09）

- **テーマ**: あなたの仕事に寄り添った、仕組みづくり
- **入口キーワード（検索）**: エクセル作成代行 / ワード作成代行 を title に残す
- **副次キーワード**: Excel作成代行 は title/H1 ではなく、description・本文・構造化データで「エクセル（Excel）」として自然に共起させる
- **サービス名**: 仕事を楽にする相談 / 仕事の仕組みづくり / 使い方サポート（「コンサル」「レッスン」は使わない）
- **ターゲット**: 中小企業の経理・事務
- **CV（デュアル）**: 資料の作成代行 or 仕事の仕組みづくり（相談）
- **サイト名（og:site_name / application-name）**: `たすかるワーク｜あなたの仕事に寄り添った仕組みづくり`
- **ナビCTA**: 相談・見積もり / トップ相談ボタン: 仕事のお困りごとを相談する
